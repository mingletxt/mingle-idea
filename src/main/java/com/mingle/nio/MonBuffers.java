...
public class MonBuffers {
    ...
    public static void main(String args[]) throws Exception {
        ...
        VirtualMachine vm = VirtualMachine.attach("88422");
        ...
        Set<ObjectName> mbeans = server.queryNames(new ObjectName("java.nio:type=BufferPool,*"), null);
        List<BufferPoolMXBean> pools = new ArrayList<BufferPoolMXBean>();
        for (ObjectName name : mbeans) {
            BufferPoolMXBean pool = ManagementFactory.newPlatformMXBeanProxy(server, name.toString(), BufferPoolMXBean.class);
            pools.add(pool);
        }
